...
export default function CadastroAvaliacaoScreen({ alunos, disciplinas, onSalvar, onCancelar }) {
  ...
  function salvar() {
    if (!alunoSelecionado || !disciplinaSelecionada || !descricao || !data || !nota) {
      alert('Preencha todos os campos obrigatórios (*)');
      return;
    }

    const novaAvaliacao = {
      id_avaliacoes: Date.now().toString(),
      id_alunos: alunoSelecionado.id,
      id_disciplinas: disciplinaSelecionada.id_disciplinas,
      descricao,
      data,
      nota,
    };

    onSalvar(novaAvaliacao);
  }
