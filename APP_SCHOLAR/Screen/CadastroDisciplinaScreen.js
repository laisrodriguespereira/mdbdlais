...
export default function CadastroDisciplinaScreen({ onSalvar, onCancelar }) {
  const [nome, setNome] = useState('');
  const [cargaHoraria, setCargaHoraria] = useState('');
  const [idCursos, setIdCursos] = useState('');
  const [idProfessores, setIdProfessores] = useState('');
  ...
  function salvar() {
    ...
    const novaDisciplina = {
      id_disciplinas: Date.now().toString(),
      nome: nome,
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.form}>
        ...
        <TextInput
          style={styles.input}
          placeholder="Digite o id_professores"
          value={idProfessores}
          onChangeText={setIdProfessores}
          keyboardType="numeric"
        />
