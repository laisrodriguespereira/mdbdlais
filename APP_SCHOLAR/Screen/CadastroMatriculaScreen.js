...
export default function CadastroMatriculaScreen({ alunos, turmas, onSalvar, onCancelar }) {
  ...
  function salvar() {
    if (!alunoSelecionado || !turmaSelecionada || !dataMatricula || !situacao) {
      alert('Preencha todos os campos obrigatórios (*)');
      return;
    }

    const novaMatricula = {
      id_matriculas: Date.now().toString(),
      id_alunos: alunoSelecionado.id,
      id_turmas: turmaSelecionada.id_turmas,
      situacao_da_matricula: situacao,
      data_matricula: dataMatricula,
    };

    onSalvar(novaMatricula);
  }
