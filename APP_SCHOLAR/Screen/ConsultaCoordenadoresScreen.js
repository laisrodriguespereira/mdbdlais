...
export default function ConsultaCoordenadoresScreen({
  ...
  const coordenadoresFiltrados = coordenadores.filter((coordenador) => {
    ...
    return (
      coordenador.nome.toLowerCase().includes(termo) ||
      coordenador.cpf.includes(termo)
    );
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.conteudo}>
        ...
        <Pressable style={styles.botaoNovo} onPress={onNovoCoordenador}>
          <Ionicons name="add" size={18} color="#FFFFFF" />
          <Text style={styles.botaoNovoTexto}>Novo Coordenador</Text>
        </Pressable>

        <FlatList
          data={coordenadoresFiltrados}
          keyExtractor={(item) => item.id_coordenadores}
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              <Pressable onPress={() => onEditarCoordenador(item)} style={{ marginRight: 12 }}>
                <Ionicons name="pencil" size={20} color="#2563EB" />
              </Pressable>
              <Pressable onPress={() => onExcluirCoordenador(item.id_coordenadores)}>
