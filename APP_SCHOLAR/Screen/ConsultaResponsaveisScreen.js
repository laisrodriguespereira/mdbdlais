...
export default function ConsultaResponsaveisScreen({
  ...
  const responsaveisFiltrados = responsaveis.filter((responsavel) => {
    ...
    return (
      ...
      responsavel.cpf.includes(termo)
    );
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.conteudo}>
        ...
        <Pressable style={styles.botaoNovo} onPress={onNovoResponsavel}>
          <Ionicons name="add" size={18} color="#FFFFFF" />
          <Text style={styles.botaoNovoTexto}>Novo Responsável</Text>
        </Pressable>

        <FlatList
          data={responsaveisFiltrados}
          keyExtractor={(item) => item.id_responsaveis}
          renderItem={({ item }) => (
            <View style={styles.card}>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardNome}>{item.nome}</Text>
                <Text style={styles.cardInfo}>CPF: {item.cpf}</Text>
                <Text style={styles.cardInfo}>Parentesco: {item.parentesco}</Text>
              </View>
              <Pressable onPress={() => onEditarResponsavel(item)} style={{ marginRight: 12 }}>
                <Ionicons name="pencil" size={20} color="#2563EB" />
              </Pressable>
              <Pressable onPress={() => onExcluirResponsavel(item.id_responsaveis)}>
