  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.conteudo}>
        ...
        <Pressable style={styles.botaoNovo} onPress={onNovoCurso}>
        ...
        <FlatList
          ...
          keyExtractor={(item) => item.id_cursos}
          renderItem={({ item }) => (
            <View style={styles.card}>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardNome}>{item.nome}</Text>
                <Text style={styles.cardInfo}>Duração: {item.duracao} anos</Text>
                <Text style={styles.cardInfo}>Carga horária: {item.carga_horaria}h</Text>
              </View>
              <Pressable onPress={() => onEditarCurso(item)} style={{ marginRight: 12 }}>
                <Ionicons name="pencil" size={20} color="#2563EB" />
              </Pressable>
              <Pressable onPress={() => onExcluirCurso(item.id_cursos)}>
