...
export default function EditarBoletimScreen({ boletim, matriculas, alunos, turmas, disciplinas, onSalvar, onCancelar, onExcluir }) {
  const matriculaAtual = matriculas.find((m) => m.id_matriculas === boletim.id_matriculas) || null;

  const [matriculaSelecionada, setMatriculaSelecionada] = useState(matriculaAtual);
  const [mediaFinal, setMediaFinal] = useState(boletim.media_final != null ? String(boletim.media_final) : '');
  const [frequencia, setFrequencia] = useState(boletim.frequencia != null ? String(boletim.frequencia) : '');
  const [situacaoFinal, setSituacaoFinal] = useState(boletim.situacao_final || 'Aprovado');
  const [faltasPorDisciplina, setFaltasPorDisciplina] = useState(boletim.faltasPorDisciplina || []);
  ...
  function salvar() {
    const boletimAtualizado = {
      ...boletim,
      id_matriculas: matriculaSelecionada ? matriculaSelecionada.id_matriculas : boletim.id_matriculas,
      media_final: mediaFinal,
      frequencia,
      situacao_final: situacaoFinal,
      faltasPorDisciplina,
    };
    onSalvar(boletimAtualizado);
  }

  function confirmarExclusao() {
    Alert.alert('Excluir boletim', 'Deseja excluir este boletim?', [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Excluir', style: 'destructive', onPress: () => onExcluir(boletim.id_boletins) },
    ]);
  }
